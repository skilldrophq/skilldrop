...
const install = Command.make(
  "install",
  {
    ...
    agent,
    scope,
    yes: Flag.boolean("yes").pipe(Flag.withAlias("y"), Flag.withDescription("Accept detected defaults and skip confirmation")),
  ...
  Effect.fn("installCommand")(function*({ snapshot, agent: requestedNames, scope: requestedScope, yes, copy }) {
    ...
    if (requestedNames.length > 0) {
      ...
      if (yes) {
        const automatic = detected.length > 0
          ? detected
          : []
        selected = [...automatic, ...definitions.filter((candidate) => candidate.name === "universal")]
      } else {
    ...
    const mode = copy || selection.targetRoots.length === 1 ? "copy" : "symlink"
