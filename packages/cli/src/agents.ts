...
export const makeAgentTopology = Effect.fn("makeAgentTopology")(
  function* (input: {
    ...
    const definitions: ReadonlyArray<Agent> = [
      {
        ...
        globalSkillsDir: path.join(input.home, ".aider-desk/skills"),
        detectionPath: path.join(input.home, ".aider-desk"),
      },
      {
        name: "claude-code",
        displayName: "Claude Code",
        projectSkillsDir: ".claude/skills",
        globalSkillsDir: path.join(input.home, ".claude/skills"),
        detectionPath: path.join(input.home, ".claude"),
      },
      {
        name: "codex",
        displayName: "Codex",
        projectSkillsDir: ".agents/skills",
        globalSkillsDir: path.join(input.home, ".agents/skills"),
        detectionPath: path.join(input.home, ".codex"),
      },
      {
        name: "cursor",
        displayName: "Cursor",
        projectSkillsDir: ".agents/skills",
        globalSkillsDir: path.join(input.home, ".agents/skills"),
        detectionPath: path.join(input.home, ".cursor"),
      },
      {
        name: "gemini-cli",
        ...
        projectSkillsDir: ".agents/skills",
        globalSkillsDir: path.join(input.home, ".agents/skills"),
        detectionPath: path.join(input.home, ".gemini"),
      },
      {
        name: "github-copilot",
        ...
        projectSkillsDir: ".agents/skills",
        globalSkillsDir: path.join(input.home, ".agents/skills"),
        detectionPath: path.join(input.home, ".copilot"),
      },
      {
        name: "goose",
        displayName: "Goose",
        projectSkillsDir: ".goose/skills",
        globalSkillsDir: path.join(input.configHome, "goose/skills"),
        detectionPath: path.join(input.configHome, "goose"),
      },
      {
        name: "opencode",
        ...
        projectSkillsDir: ".agents/skills",
        globalSkillsDir: path.join(input.home, ".agents/skills"),
        detectionPath: path.join(input.configHome, "opencode"),
      },
      {
        name: "openhands",
        displayName: "OpenHands",
        projectSkillsDir: ".openhands/skills",
        globalSkillsDir: path.join(input.home, ".openhands/skills"),
        detectionPath: path.join(input.home, ".openhands"),
      },
      {
        name: "universal",
    ...
    const roots = new Map<
      string,
      { path: string; scope: SkillScope; agents: Array<Agent> }
    >();
...
export const loadAgentTopology = Effect.fn("loadAgentTopology")(function* () {
  ...
  const home = yield* Config.string("HOME").pipe(
    Effect.mapError(
      () => new CliError({ message: "Could not determine the home directory" }),
    ),
  );
...
export const detectInstalledAgents = Effect.fn("detectInstalledAgents")(
  function* (topology: AgentTopology) {
    const fs = yield* FileSystem.FileSystem;
    const detected: Array<Agent> = [];
    for (const agent of topology.definitions) {
      if (
        agent.name !== "universal" &&
        (yield* fs
          .exists(agent.detectionPath)
...
export const resolveAgentSelection = (
  ...
  const targetRoots = topology.roots.flatMap((root) =>
    root.scope === scope &&
    root.agents.some((agent) => selectedNames.has(agent.name))
      ? [root.path]
      : [],
  );
  return {
