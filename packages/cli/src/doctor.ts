...
const writableTargetCheck = Effect.fn("writableTargetCheck")(function* (
  label: string,
  target: string,
) {
...
export const runDoctor = Effect.fn("runDoctor")(function* () {
  ...
  checks.push(
    yield* writableTargetCheck(
      "Project skills",
      topology.canonicalRoots.project,
    ),
  );
  checks.push(
    yield* writableTargetCheck("Global skills", topology.canonicalRoots.global),
  );
  return checks;
});
