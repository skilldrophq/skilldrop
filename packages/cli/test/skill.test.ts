...
describe("skill bundles", () => {
  ...
  test("honors .skillignore and excludes the ignore file from the bundle", async () => {
    const files = await run(Effect.gen(function*() {
      const fs = yield* FileSystem.FileSystem
      const path = yield* Path.Path
      const root = yield* fs.makeTempDirectoryScoped({ prefix: "ignored-skill" })
      yield* fs.writeFile(path.join(root, "SKILL.md"), encoder.encode("# Ignored files\n"))
      yield* fs.writeFile(path.join(root, ".env"), encoder.encode("SECRET=do-not-share\n"))
      yield* fs.writeFile(path.join(root, ".skillignore"), encoder.encode([
        ".env",
        "fixtures/",
        "*.private",
      ...
      yield* fs.writeFile(path.join(root, "fixtures", "customer.json"), encoder.encode("{}\n"))
      yield* fs.makeDirectory(path.join(root, "references"))
      yield* fs.writeFile(path.join(root, "references", "public.md"), encoder.encode("Public\n"))
      yield* fs.writeFile(path.join(root, "references", "notes.private"), encoder.encode("Private\n"))
      yield* fs.writeFile(path.join(root, "references", "keep.private"), encoder.encode("Keep\n"))
      yield* fs.writeFile(path.join(root, "root-only.txt"), encoder.encode("Ignored at root\n"))
      yield* fs.writeFile(path.join(root, "references", "root-only.txt"), encoder.encode("Kept when nested\n"))
      yield* fs.writeFile(path.join(root, "secrets.json"), encoder.encode("{}\n"))
  ...
  test("rejects symlinks that point outside the skill root", async () => {
    const error = await run(Effect.gen(function*() {
      const fs = yield* FileSystem.FileSystem
      const path = yield* Path.Path
      const root = yield* fs.makeTempDirectoryScoped({ prefix: "linked-skill" })
      const outside = yield* fs.makeTempDirectoryScoped({ prefix: "private-fixture" })
      yield* fs.writeFile(path.join(root, "SKILL.md"), encoder.encode("# Linked skill\n"))
      yield* fs.writeFile(path.join(outside, ".env"), encoder.encode("SECRET=do-not-share\n"))
      yield* fs.symlink(path.join(outside, ".env"), path.join(root, "leaked.env"))
