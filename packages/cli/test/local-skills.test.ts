...
import { dim, discoverInstalledSkills, renderInstalledSkills } from "../src/local-skills.ts"
...
describe("installed skill discovery", () => {
  test("finds skills in project and global roots and ignores other directories", async () => {
    const skills = await run(Effect.gen(function*() {
      ...
      const project = yield* fs.makeTempDirectoryScoped({ prefix: "skilldrop-project" })
      const global = yield* fs.makeTempDirectoryScoped({ prefix: "skilldrop-global" })
      yield* fs.makeDirectory(path.join(project, "review-pr"), { recursive: true })
      yield* fs.writeFile(path.join(project, "review-pr", "SKILL.md"), encoder.encode("# Review PR\n"))
      yield* fs.makeDirectory(path.join(project, "not-a-skill"), { recursive: true })
      yield* fs.makeDirectory(path.join(global, "explain-code"), { recursive: true })
      yield* fs.writeFile(path.join(global, "explain-code", "SKILL.md"), encoder.encode("# Explain code\n"))
  ...
  test("sorts project before global when a skill exists in both scopes", async () => {
    const skills = await run(Effect.gen(function*() {
      ...
      for (const root of [project, global]) {
        yield* fs.makeDirectory(path.join(root, "review-pr"), { recursive: true })
        yield* fs.writeFile(path.join(root, "review-pr", "SKILL.md"), encoder.encode("# Review PR\n"))
      }
      return yield* discoverInstalledSkills([
        { path: global, scope: "global", agents: [] },
        { path: project, scope: "project", agents: [] }
  ...
  test("deduplicates the same physical skill root", async () => {
    const skills = await run(Effect.gen(function*() {
      const fs = yield* FileSystem.FileSystem
      const path = yield* Path.Path
      const root = yield* fs.makeTempDirectoryScoped({ prefix: "skilldrop-shared" })
      yield* fs.makeDirectory(path.join(root, "review-pr"), { recursive: true })
      yield* fs.writeFile(path.join(root, "review-pr", "SKILL.md"), encoder.encode("# Review PR\n"))
      return yield* discoverInstalledSkills([
  ...
  test("groups the list by agent and scope and renders dimmed prompt metadata", () => {
    const output = renderInstalledSkills([
      { name: "review-pr", path: "/repo/.agents/skills/review-pr", scope: "project", agents: [universal] },
      { name: "docs", path: "/home/me/.claude/skills/docs", scope: "global", agents: [claude] }
    ])
  ...
  test("renders the same scoped skill once across multiple agents", () => {
    const output = renderInstalledSkills([
      { name: "review-pr", path: "/home/me/.claude/skills/review-pr", scope: "global", agents: [claude] },
      { name: "review-pr", path: "/home/me/.agents/skills/review-pr", scope: "global", agents: [universal] }
    ])

    expect(output).toContain("Installed skills (1)")
    expect(output).toContain("Claude Code + Universal\n  global\n    review-pr")
